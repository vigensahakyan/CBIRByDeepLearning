...
public class AutoEncoder extends NeuralNetworkBase {
	...
	public static void main(String[] args) throws IOException {
		...
		nn.InitializeTrainingData("path/to/mnistnorm.csv");
		nn.firstWeightActivation();
		nn.learnNNByStohasticGradientDescent(45);
		try {
			File file = new File("path/to/AutoEncoder.ser");
			file.createNewFile();
			...
			out.close();
			fileOut.close();
			System.out.println("Serialized data is saved in path/to/autoencoder.ser");
		} catch (IOException i) {
			i.printStackTrace();
		}
		...
		nn.writeFeatureResult("path/to/featureResult.csv");
		nn.encode();
		System.out.println("write to file");
		nn.writeResult("path/to/mnistencode.csv");
		nn.InitializeTrainingData("path/to/mnist_testnorm.csv");
		nn.encode();
		System.out.println("write to file");
		nn.writeResult("path/to/mnistTestencode.csv");
